...
object DirectoryIteratorOHeap extends Serializable {

  val trace = false

  /* 
  Moved into HashingTF Again for tracing
  var hash = 0

  case class wrapByteBuffer(buff: ByteBuffer) {

    def canEqual(that: Any) = that.isInstanceOf[wrapByteBuffer]
    ...
    override def hashCode(): Int = {
      ...
      //  this won't work with direct ByteBuffers
            //  (buff.hasArray) LEFTBRACE
              //  byteArray = buff.array()
              //  (myByte <- byteArray)  LEFTBRACE
                  //  = 31 * h + myByte.toChar;
              // 
            //  hash = h;
	    // 
