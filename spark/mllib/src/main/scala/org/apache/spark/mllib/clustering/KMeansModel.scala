...
class KMeansModel @Since("1.1.0") (@Since("1.0.0") val clusterCenters: Array[Vector])
  ...
  def computeCost(data: KMeansRDD): Double = {
    val bcCentersWithNorm = data.context.broadcast(clusterCentersWithNorm)
    data.map(p => KMeans.pointCost(bcCentersWithNorm.value, p)).sum()
  }

  def computeCost(data: KMeansDirectRDD): Double = {
    val bcCentersWithNorm = data.context.broadcast(clusterCentersWithNorm)
    data.map(p => KMeans.pointCost(bcCentersWithNorm.value, p)).sum()
  }
 ...
 def createVector(data: Array[Array[Double]], index: Int, dims: Int): Vector = {
  ...
  def computeCost(
    ...
    val sc = dataIndex.context
    val bcCentersWithNorm = sc.broadcast(clusterCentersWithNorm)
    val bData = sc.broadcast(data)
    dataIndex.map(i => KMeans.pointCost(bcCentersWithNorm.value,
        new VectorWithNorm(createVector(bData.value, i, bData.value.length)))).sum()
  }
