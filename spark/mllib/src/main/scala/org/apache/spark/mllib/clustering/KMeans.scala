...
class KMeans private (
   ...
   */
  @Since("0.8.0")
  def this() = this(2, 20, true, KMeans.K_MEANS_PARALLEL, 2, 1e-4, Utils.random.nextLong())
 ...
 */
private[clustering]
class VectorWithNorm(val vector: Vector, var norm: Double) extends Serializable {
...
private[clustering]
class DenseVectorWithNorm(val vector: DenseVector, val norm: Double) extends Serializable {

  def this(vector: DenseVector) = this(vector, Vectors.norm(vector, 2.0))

  def this(array: Array[Double]) = this(new DenseVector(array))

  /** Converts the vector to a dense vector. */
  def toDense: DenseVectorWithNorm = new DenseVectorWithNorm(new DenseVector(vector.toArray), norm)
}
