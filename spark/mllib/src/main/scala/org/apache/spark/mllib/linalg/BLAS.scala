...
private[spark] object BLAS extends Serializable with Logging {
  ...
  /** Y += a * x */
  private[spark] def axpy(a: Double, data: Array[Array[Double]], i: Int, Y: DenseVector): Unit = {
    for (j <- 0 to data.length - 1) {
      Y.values(j) += a * data(j)(i)
    }
  }
   ...
   * dot(x, y)
   */
  def dot(x: DenseVector, data: Array[Array[Double]], i: Int): Double = {
    var d = 0.0;
    for (j <- 0 to x.size - 1) {
      d = d + data(j)(i) * x.values(j)
    }
    d
  }
