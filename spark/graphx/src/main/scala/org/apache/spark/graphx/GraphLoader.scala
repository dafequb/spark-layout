...
object GraphLoader extends Logging {
    ...
    : Graph[Int, Int] =
  {
    ...
    val edges = lines.mapPartitionsWithIndex { (pid, iter) =>
      ...
      iter.foreach { line =>
        if (!line.isEmpty && line(0) != '#' && !line(0).isLetter) {
  ...
  /*
   * Alternative version of edgeListFile
   */
   def myEdgeListFile(
       sc: SparkContext,
       path: String,
     ...
     : Graph[Int, Int] =
   {
     ...
     val edges = lines.mapPartitionsWithIndex { (pid, iter) =>
       val builder = new MyEdgePartitionBuilder[Int, Int]
       iter.foreach { line =>
         if (!line.isEmpty && line(0) != '#' && !line(0).isLetter) {
           val lineArray = line.split("\\s+")
           if (lineArray.length < 2) {
       ...
       Iterator((pid, builder.toEdgePartition))
     }.persist(edgeStorageLevel).setName("GraphLoader.myEdgeListFile - edges (%s)".format(path))
     edges.count()
